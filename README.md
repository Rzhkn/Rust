...
Между константами и неизменяемыми переменными есть несколько различий:
- для констант необходимо сразу же указывать тип данных и значение. А в неизменяемую переменную можно изначально просто объявить, а потом уже присвоить ей значение.
- Значение константы должно быть известно на момент компиляции, переменной же можно присваивать значение уже в период работы программы
- Разрешается объявлять глобальные константы, а вот глобальные переменные создавать нельзя 
...
Получить же значение мы можем двумя способами:  
  ...
  2. Обращаться к элементам через точку:
    ...
    ```
В Rust нет неявного преобразования типов данных. Даже если мы захотим переменной большего типа присвоить переменную меньшего (например, из `i8` в `i32`), компилятор все равно выдаст ошибку. Но в ситуациях, когда данное преобразование необходимо используется `as` (может привести к неккоректным значениям):
``` Rust
let a: i32 = 8;
println!("a={a}");
let b: i8 = a as i8;
...
Анонимные функции - это такие же функции, только без названия и имеют возможность сохраняться в переменную.  
Пример:
```Rust
let sqrt = |x: i32| //если бы в функц не было параметров, нужно было бы указать пустые ||
{ 
    let res = x * x;
    println!("{}^2={}",x,res);
};
...
sqrt(4); // 4^2=16
```
Анонимная функция также может возвращать значение.  
К тому же в анонимных функциях можно не указывать тип переменных, единственное, тип автоматически определяется при первом вызове функции, так что вызвать ее сначала для целочисленного типа, а потом для дробного не получится.  
Еще отличительной чертой анонимных функций является то, что если функция содержит лишь одно выполняемое действие, то фигурные скобки можно опустить.
```Rust
let sum = |a,b| a+b;
let s1=sum(1,2);
//Ошибка, тк функция ждет целочисленные переменные
//let s2=sum(2.3,3.4);
```
...
- match
    `match` используется, когда у нас выражение принимает различные значения от которых зависит полученный результат (аналог switch). Пример:
    ```Rust
    let a = 1;
    let b = match a {
        1=>"один",
        2=>"два",
        _=>"неизвестно" //необязательно указывать; выполняется, когда ни один из вариантов выше не подошел
    };
    ```
